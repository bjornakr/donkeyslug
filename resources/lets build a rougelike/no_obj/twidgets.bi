...
Sub InitWidgets ()
   ...
   If ScreenPtr Then
      'Get the current graphics width, height and color depth.
...
Function tInputbox.InputBox(result As String) As btnID
   ...
   If _win.InitOK = tTrue Then
      ...
      Loop Until done = tTrue
   End If
   'Destroy window and restore background.
   _win.DestroyWindow
...
'******************************************************************************************************
'List Object
'Displays a list on the screen and allows the user to select the item from the list.
'******************************************************************************************************
Type listtype
   id As Integer
   text As String
...
Type tList
   ...
   _prompt As String           'The prompt to display.
   _promptcolor As UInteger    'Color of the message.
   _hilite As UInteger         'Color of the selected item in the list.
   _btn1 As tButton            'Ok buton.
   _btn2 As tButton            'Cancel button.
   Public:
   Declare Constructor ()
...
'Sets the list row.
Property tList.Row(r As Integer)
   _row = r
End Property

'Returns the list row.
Property tList.Row() As Integer
   Return _row
End Property

'Sets the list column.
Property tList.Col(c As Integer)
   _col = c
End Property

'Returns the list column.
Property tList.Col() As Integer
   Return _col
End Property

'Sets the window line style.
Property tList.WindowStyle (wstyle As wlinestyle)
   _win.LineStyle = wstyle
End Property

'Sets background color.
Property tList.BackColor (clr As UInteger)
   _win.BackColor = clr
End Property

'Returns the background color.
Property tList.BackColor () As UInteger
   Return _win.Backcolor
End Property

'Sets the border color.
Property tList.BorderColor (clr As UInteger)
   _win.BorderColor = clr
End Property

'Returns the border color.
Property tList.BorderColor () As UInteger
   Return _win.BorderColor
End Property

'Sets the color of the selected button.
Property tList.ButtonSelectColor (clr As UInteger)
   _btnselcolor = clr
End Property

'Returns the color of the selected button.
Property tList.ButtonSelectColor () As UInteger
   Return _btnselcolor
End Property

'Sets the color of the non-selected button.
Property tList.ButtonColor (clr As UInteger)
   _btncolor = clr
End Property

'Returns the color of the non-selected button.
Property tList.ButtonColor () As UInteger
   Return _btncolor
End Property

'Sets the prompt string.
Property tList.Prompt (s As String)
   _prompt = s
End Property

'Returns the prompt string.
Property tList.Prompt () As String
   Return _prompt
End Property

'Sets the prompt forecolor.
Property tList.PromptColor (clr As UInteger)
   _promptcolor = clr
End Property

'Returns the prompt forecolor.
Property tList.PromptColor () As UInteger
   Return _promptcolor
End Property

'Sets the window title.
Property tList.Title(s As String)
   _win.Title = s
End Property

'Returns the window title.
Property tList.Title() As String
   Return _win.Title
End Property

'Sets the window title alignment.
Property tList.AlignTitle(t As wtitlealign)
   _win.AlignTitle = t
End Property

'Returns the window title alignment.
Property tList.AlignTitle() As wtitlealign
   Return _win.AlignTitle
End Property

'Sets the window title color.
Property tList.TitleColor(c As UInteger)
   _win.TitleColor = c
End Property

'Returns the window title color.
Property tList.TitleColor() As UInteger
   Return _win.TitleColor
End Property

'Sets the shadow drawing on and off.
Property tList.Shadow (s As Integer)
   _win.Shadow = s
End Property

'Resturns current shadow flag setting.
Property tList.Shadow () As Integer
   Return _win.Shadow
End Property

'Displays list and returns selected item id.
Function tList.Listbox (lines() As listtype, ByRef id As integer) As btnID
   Dim As btnID btn, activebtn
   Dim As String btn1, btn2, btn3, ch, prcopy = _prompt
   Dim As Integer lprompt, row1, col1, row2, col2, lncnt
   Dim As Integer prow, pcol, btnrow, btn1col, btn2col
   Dim As Integer bidx1, bidx2, idx = LBound(lines), savrow
   ...
   If _win.InitOK = tTrue Then
      ...
      For i As Integer = LBound(lines) To UBound(lines)
         ...
         lncnt += 1
         'Get longest line.
         If Len(lines(i).text) > lprompt Then
            lprompt = Len(lines(i))
         EndIf
      Next
      'Draw the box.
      ...
      For i As Integer = LBound(lines) To UBound(lines)
         If i = idx Then
            PutText lines(i).text, prow, pcol, _hilite
         Else
            PutText lines(i).text, prow, pcol, _promptcolor
         EndIf
         prow += 1
      Next
      ScreenUnlock
      ...
      Do
         ...
         EndIf
         Sleep 1
      Loop Until (btn = gbnOK) Or (btn = gbnCancel)
   End If
   'Destroy window and restoe background.
   _win.DestroyWindow
