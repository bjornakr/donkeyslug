...
'******************************************************************************************************
Dim Shared gBackColor As UInteger
Dim Shared gBackChar As String


'******************************************************************************************************
'System Object
'This contains the system information such as rows and columns and color depth.
'******************************************************************************************************
Type tsys
   Private:
   _initok As Integer          'Indicates that the window initialized properly.
   _srows As Integer           'Screen text rows.
   _scols As Integer            'Screen text columns.
   _charw As Integer            'The width of the screen characters.
   _charh As Integer            'The height of the screen characters.
   _depth As Integer            'Graphics color depth.
   _gw As Integer               'Graphics width.
   _gh As Integer               'Graphics height.
   Public:
   Declare Constructor ()
   Declare Property InitOK () As Integer       'Indicates that the system initialized properly.
   Declare Property ScreenRows() As Integer    'The number of rows on the screen.
   Declare Property ScreenCols() As Integer    'The number of columns on the screen. 
   Declare Property CharWidth() As Integer     'The character width.
   Declare Property CharHeight() As Integer    'The character height.
   Declare Property ColorDepth() As Integer    'The screen color depth.
   Declare Property GrWidth() As Integer       'The graphics width of the screen.
   Declare Property GrHeight() As Integer      'The graphics height of the screen.
   Declare Sub PutText(txt As String, row As Integer, col As Integer, clr As UInteger)
   Declare Sub PutTextShadow(txt As String, row As Integer, col As Integer, clr As UInteger)
   Declare Function tCenterX (txt As String) As Integer
   Declare Function tCenterX (lin As Integer) As Integer
   Declare Function tCenterX (col1 As Integer, col2 As Integer, txt As String) As Integer
   Declare Function tCenterY (lins As Integer) As Integer
End Type
...
Property tsys.InitOK () As Integer
   Return _initok
End Property

Property tsys.ScreenRows() As Integer
   Return _srows
End Property

Property tsys.ScreenCols() As Integer
   Return _scols
End Property

Property tsys.CharWidth() As Integer
   Return _charw
End Property

Property tsys.CharHeight() As Integer
   Return _charh
End Property

Property tsys.ColorDepth() As Integer
   Return _depth
End Property

Property tsys.GrWidth() As Integer
   Return _gw
End Property

Property tsys.GrHeight() As Integer
   Return _gh
End Property

'Draws text at specified row and column.
Sub tsys.PutText(txt As String, row As Integer, col As Integer, clr As UInteger)
	Dim As Integer x, y
...
Sub tsys.PutTextShadow(txt As String, row As Integer, col As Integer, clr As UInteger)
	Dim As Integer x, y
...
Function tsys.tCenterX (txt As String) As Integer
   Return (_scols / 2) - (Len(txt) / 2)
End Function

'Centers text on screen.
Function tsys.tCenterX (lin As Integer) As Integer
   Return (_scols / 2) - (lin / 2)
End Function

'Centers length of text between col1 and col2.
Function tsys.tCenterX (col1 As Integer, col2 As Integer, txt As String) As Integer
   Dim As Integer c
...
Function tsys.tCenterY (lins As Integer) As Integer
   Return (_srows / 2) - (lins / 2)
End Function
...
Constructor tButton ()
   ...
   If _sys.InitOk = tTrue Then
      _initok = tTrue
      _text = ""                          'Button caption.
      _selected = tFalse                  'Selected flag.
      _shadow = tTrue                     'Default shadow flag.
      If _sys.ColorDepth > 8 Then         'Selected color.
         _btnselcolor = RGB(255, 255, 0)
      Else
         _btnselcolor = 14
      End If
      If _sys.ColorDepth > 8 Then         'Non-selected color.
...
Sub tButton.DrawButton ()
   ...
   If _initok = tTrue Then
      ...
      ebutt = String(Len(_text), gBackChar)
      _sys.PutText ebutt, _row, _col, gBackColor
      'Draw the shadow.
      If _shadow = tTrue Then
         'Draw the button.
         _sys.PutTextShadow butt, _row, _col, clr
      Else
         'Draw the button.
         _sys.PutText butt, _row, _col, clr
      EndIf
   EndIf
End Sub
...
'Calling DestroyWindow restores the background. Uses tsys for system information.
'******************************************************************************************************
...
Constructor tWin
   ...
   If _initok = tTrue Then
      ...
      _row1 = 1
      _row2 = _sys.ScreenRows - 2
      _col1 = 1
      _col2 = _sys.ScreenCols - 2
      ...
      If _sys.ColorDepth > 8 Then
         _backcolor = RGB (0, 0, 128)
         gBackColor = RGB (0, 0, 128)
...
Sub twin._DrawWindow()    
   ...
   Dim As String lne, wtitle = _title, ecl, ecr
	...
	If _initok = tTrue Then
	   ...
	   If _col2 > _sys.ScreenCols Then _col2 = _sys.ScreenCols - 1
	   If _row1 < 1 Then _row1 = 1
	   If _row2 > _sys.ScreenRows Then _row2 = _sys.ScreenRows - 1
      'Save the background under window.
      _x1 = (_col1 - 1) * _sys.CharWidth
      _x2 = ((_col2 + 2) - 1) * _sys.CharWidth
      _y1 = (_row1 - 1) * _sys.CharHeight
      _y2 = ((_row2 + 2) - 1) * _sys.CharHeight
      'Force the x,y to screen.
      If _x1 < 0 Then _x1 = 0
      If _x2 > _sys.GrWidth - 1 Then _x2 = _sys.GrWidth - 1 
      If _y1 < 0 Then _y1 = 0
      If _y2 > _sys.GrHeight - 1 Then _y2 = _sys.GrHeight - 1 
      'Grab the background.
      ...
      For i = _row1 To _row2
         _sys.PutText lne, i, _col1, _backcolor
      Next
      'Print the top row.
      lne = _tlc
      lne &= String((_col2 - _col1) - 1, _tbl)
      lne &= _trc
      _sys.PutText lne, _row1, _col1, _borderfore
      'Print the sides.
      lne = _rll
      For i = _row1 + 1 To _row2 - 1
         _sys.PutText lne, i, _col1, _borderfore
         _sys.PutText lne, i, _col2, _borderfore
      Next
      'Print bottom row.
      lne = _blc
      lne &= String((_col2 - _col1) - 1, _tbl)
      lne &= _brc
      _sys.PutText lne, _row2, _col1, _borderfore
      ...
      If Len(wtitle) > 0 Then
         ...
         'Center title.   
         Else
            tx = _sys.tCenterX(_col1, _col2, ecl & wtitle & ecr)
         EndIf
      EndIf
      'Draw the title.
      If _shadow = tTrue Then
         'Make a space for the title.
         _sys.PutText String(Len(ecl & wtitle & ecr), _bkc), _row1, tx, _backcolor
         'Draw the title.
         _sys.PutText ecl, _row1, tx, _borderfore
         _sys.PutTextShadow wtitle, _row1, tx + 2, _titlecolor 
         _sys.PutText ecr, _row1, tx + 2 + Len(wtitle), _borderfore
      Else
         'Make a space for the title.
         _sys.PutText String(Len(ecl & wtitle & ecr), _bkc), _row1, tx, _backcolor
         'Draw the title.
         _sys.PutText ecl, _row1, tx, _borderfore
         _sys.PutText wtitle, _row1, tx + 2, _titlecolor 
         _sys.PutText ecr, _row1, tx + 2 + Len(wtitle), _borderfore
      EndIf
      ...
      If _shadow = tTrue Then
         ...
         If _sys.ColorDepth > 8 Then
            _sys.PutText lne, _row2 + 1, _col1 + 1, RGB(0, 0, 0)
         Else
            _sys.PutText lne, _row2 + 1, _col1 + 1, 0
         End If
         'Draw shadow along the side.
         lne = Chr(177)
         For i = _row1 + 1 To _row2
            If _sys.ColorDepth > 8 Then
               _sys.PutText lne, i, _col2 + 1, RGB(0, 0, 0)
            Else
               _sys.PutText lne, i, _col2 + 1, 0
            End If
         Next
      End If
	End If
...
Function tMsgbox.MessageBox(prompt As String) As btnID
   ...
   If _win.InitOK = tTrue Then
      ...
      prow = _sys.tCenterY(6) - 2
      pcol = _sys.tCenterX(prcopy)
      'Build the box around the prompt.
      row1 = prow - 2
      row2 = prow + 4
      col1 = _sys.tCenterX(lprompt) - 2
      col2 = col1 + lprompt + 3
      ...
      If _msgstyle = gmbOK Then
         ...
         _btn1.SelText = "< OK >"
         _btn1.Row = btnrow
         _btn1.Col = _sys.tCenterX(col1, col2, btn1)
      ...
      If _msgstyle = gmbOKCancel Then
         btn1 = "[ OK ]"
         btn2 = " [ Cancel ]"
         btn1col = _sys.tCenterX(col1, col2, btn1 & btn2)
      ...
      If _msgstyle = gmbYesNoCancel Then
         ...
         btn2 = " [ No ]"
         btn3 = " [ Cancel ]" 
         btn1col = _sys.tCenterX(col1, col2, btn1 & btn2 & btn3)
         btn2col = btn1col + Len(btn1)
      ...
      _win.DrawWindow
      'Draw the prompt.
      _sys.PutText prcopy, prow, pcol, _promptcolor
      ScreenUnlock
      'Get the input.
      Do
         ch = Inkey
...
Type tEdit
   ...
   _maxlen As Integer                          'Maximum length of the text. 0 = unlimited.
   ...
   Declare Property MaxLen(m As Integer)       'Sets the max length of input text. 0 = unlimited.
   Declare Property MaxLen() As Integer        'Returns the max length of input text. 0 = unlimited.
   Declare Property InputLen (i As Integer)    'Sets the length of the input field. Default is 10 chars. 
   Declare Property InputLen () As Integer     'Returns the length of the input field. Default is 10 chars.
...
Function tEdit._Paint(tstart As Integer, tend As Integer) As Integer
  ...
  'Erase the edit field.
   ifield = String(_inputlen, gBackChar)
   _sys.PutText ifield, _row, _col, gBackColor
   'Draw the edit field.
   ifield = String(_inputlen, _inputchar)
   _sys.PutText ifield, _row, _col, _inputcolor
   'Get the text view if we have text.
   If Len(_text) > 0 Then
      vtxt = _Substr(tstart, tend)
       _sys.PutText vtxt, _row, _col, _textcolor
       ret = Len(vtxt) + 1
   Else
      ret = 1
   EndIf
...
Constructor tEdit ()
   If _sys.InitOk = tTrue Then
      If _sys.ColorDepth > 8 Then        'Default colors.
...
'Sets the max length of input text. 0 = unlimited.
Property tEdit.MaxLen(m As Integer)
   _maxlen = m
End Property

'Returns the max length of input text. 0 = unlimited.
Property tEdit.MaxLen() As Integer
   Return maxlen
End Property

'Sets the length of the input field. Default is 10 chars.
Property tEdit.InputLen (i As Integer)
   _inputlen = i
End Property

'Returns the length of the input field. Default is 10 chars.
...
Function tEdit.DrawEdit () As tKeys    
   ...
   If _sys.InitOk = tTrue Then
      ...
      ccol = (_col - 1) + tidx 
      'Draw the caret.
      _sys.PutText _caret, _row, ccol, _caretcolor
      Do
         ch = InKey
         If ch <> "" Then
            If ch = KeyEnter Then
...
Type tInputbox
   ...
   _inputlen As Integer        'Input field length.
   Public:
   Declare Constructor ()
...
Function tInputbox.InputBox(result As String) As btnID
   ...
   If _win.InitOK = tTrue Then
      'Set the prompt length to the maxlen size if less than maxlen.
      ...
      _edit.Row = prow + 2
      _edit.Col = _sys.tCenterX(lprompt)
