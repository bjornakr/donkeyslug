...
Sub levelobj.MoveMonsters ()
   ...
   For i As Integer = 1 To _level.nummon
      ...
      If (_level.moninfo(i).isdead = FALSE) And (_level.moninfo(i).effects(meStun).cnt < 1) Then
...
sub levelobj.DoTimedEvents()
   ...
   For i As Integer = 1 To _level.nummon
      ...
      If _level.moninfo(i).isdead = FALSE Then
         For j As monSpells = mePoison To meStun
            If _level.moninfo(i).effects(j).cnt > 0 Then
               _level.moninfo(i).currhp -= _level.moninfo(i).effects(j).dam
               _level.moninfo(i).effects(j).cnt -= 1
               If _level.moninfo(i).currhp < 1 Then
                  _level.moninfo(i).isdead = TRUE
                  pchar.CurrXP = pchar.CurrXP + _level.moninfo(i).xp
                  pchar.TotXP = pchar.TotXP + _level.moninfo(i).xp
                  txt = _level.moninfo(i).mname & " died from a spell."
                  PrintMessage txt
               EndIf
            EndIf
         Next
